...
class ArticleController extends Controller{
    ...
    public function actionUpdate_article(){
      ...
      public function actionGet_articles(){
        $articles = Article::findBySql("select * from article")->asArray()->all();
        return json_encode($articles);
    }

    public function actionGet_articles_by_author(){
        $data = \Yii::$app->request->get();
        $author = $data['author'];
        $articles = Article::findBySql("select * from article where author = :1",array(':1' =>$author))->asArray()->all();
        return json_encode($articles);
    }
    ...
    public function actionGet_article_by_id(){
        ...
        $articles = Article::findBySql($sql,array(':1'=>$id))->asArray()->all();
        return json_encode($articles);
    }
    ...
    public function actionGettypes(){
        return json_encode(Article::findBySql('select distinct type from article')->asArray()->all());
    }

    public function actionGet_article_by_type(){
        $request = \Yii::$app->request;
        $type = $request->get('type');
        $sql = 'select * from article where type=:1 order by createtime desc';
        return json_encode(Article::findBySql($sql,array(':1'=>$type))->asArray()->all());
    }
    ...
    public function behaviors()
    {
        ...
        // remove authentication filter00
